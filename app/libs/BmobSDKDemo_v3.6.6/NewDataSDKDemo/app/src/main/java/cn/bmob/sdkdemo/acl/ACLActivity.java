...
public class ACLActivity extends BaseActivity {
	...
	private void createACLData(){
		...
		person.setName("职员0508");
		person.setAddress("huizhou");
		//添加ACL权限控制
...
//		aCL.setWriteAccess("用户的objectId", true);//指定特定用户可写
		BmobRole hr1 = new BmobRole("hr1");
		aCL.setRoleReadAccess(hr1, true);//指定hr类的人可读
		aCL.setRoleWriteAccess("hr1", true);//指定hr类的人可写
		person.setACL(aCL);
		addSubscription(person.save(new SaveListener<String>() {
	...
	private void createRole(){
		...
		addSubscription(hr_role.save(new SaveListener<String>() {
			@Override
			public void done(String o, BmobException e) {
				if(e==null){
					log("创建成功");
	...
	private void updateRole() {
		...
		addSubscription(role.update(new UpdateListener() {
