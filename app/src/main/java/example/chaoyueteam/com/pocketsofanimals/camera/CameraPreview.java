package example.chaoyueteam.com.pocketsofanimals.camera;
...
import static example.chaoyueteam.com.pocketsofanimals.modules.takephoto.TakePhotoActivity.isTransverse;

public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback, android.hardware.Camera.AutoFocusCallback {
        ...
        try {
            ...
            for (int camIdx = 0; camIdx < cameraCount; camIdx++) {
                ...
                if (cameraInfo.facing == android.hardware.Camera.CameraInfo.CAMERA_FACING_BACK) {
                    try {
                        c = android.hardware.Camera.open(camIdx);   //打开后置摄像头
            ...
            if (c == null) {
                c = android.hardware.Camera.open(0); // attempt to get a Camera instance
            }
        } catch (Exception e) {
            Toast.makeText(getContext(), "摄像头打开失败！", Toast.LENGTH_SHORT).show();
        }
    ...
    private void updateCameraParameters() {
        if (camera != null) {
            ...
            try {
                ...
                android.hardware.Camera.Size previewSize = findBestPreviewSize(p);
                p.setPreviewSize(previewSize.width, previewSize.height);
               /* p.setPreviewSize(2048, 1536);*/
     ...
     * @param p
     */
    private void setParameters(android.hardware.Camera.Parameters p) {
        ...
        android.hardware.Camera.Size previewSize = findPreviewSizeByScreen(p);
        p.setPreviewSize(previewSize.width, previewSize.height);
        p.setPictureSize(previewSize.width, previewSize.height);
     ...
     * @return
     */
    private android.hardware.Camera.Size findPreviewSizeByScreen(android.hardware.Camera.Parameters parameters) {
     ...
     * @return
     */
    private android.hardware.Camera.Size findBestPreviewSize(android.hardware.Camera.Parameters parameters) {
    ...
    public void focusOnTouch(MotionEvent event) {
        ...
        if (parameters.getMaxNumFocusAreas() > 0) {
            List<android.hardware.Camera.Area> focusAreas = new ArrayList<android.hardware.Camera.Area>();
            focusAreas.add(new android.hardware.Camera.Area(focusRect, 1000));

            parameters.setFocusAreas(focusAreas);
        }

        if (parameters.getMaxNumMeteringAreas() > 0) {
            List<android.hardware.Camera.Area> meteringAreas = new ArrayList<android.hardware.Camera.Area>();
            meteringAreas.add(new android.hardware.Camera.Area(meteringRect, 1000));
